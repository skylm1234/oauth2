			...
			if (ReUtil.isMatch("^box_monry_.*$", type) ||
				...
				if (type.equals("honor")) {
					if (parameter != null && parameter.equals("archives")) {
						...
						updateRanklistHonor(redisTemplate, identifier);
						onNotifyEventOfPromotions(redisTemplate, "maxhonor", value, identifier);
					}
				}

				if (ReUtil.isMatch("^exp_book_.*$", type)) {
					onNotifyEventOfPromotions(redisTemplate, "expbooks", value, identifier);
				} else if (type.equals("gold")) {
					onNotifyEventOfPromotions(redisTemplate, "maxgold", value, identifier);
				} else {
				}
			} else if (type.equals("exp")) {
				//上阵角色都加经验
				...
				awardHeroForMe(redisTemplate, identifier, type, String.valueOf(parameter));
			} else if (type.equals("dummy")) {
				log.info("do nothing");
			} else {
